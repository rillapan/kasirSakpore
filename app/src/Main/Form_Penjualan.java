...
public class Form_Penjualan extends javax.swing.JPanel {
    ...
    try {
        String cari = "SELECT produk.kode_produk, produk.nama_produk, kategori.nama_kategori, merek.nama_merek, "
                ...
                + "JOIN merek ON produk.merek_id = merek.merek_id "
                + "WHERE CAST(produk.kode_produk AS TEXT) LIKE ? "
                + "OR produk.nama_produk ILIKE ? "
                + "OR kategori.nama_kategori ILIKE ? "
                + "OR merek.nama_merek ILIKE ?";

        ps = Koneksi.getConnection().prepareStatement(cari);
        String keyword = "%" + txt_cariBarang.getText() + "%";
    ...
    private void txt_jumlahBarangKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_jumlahBarangKeyReleased
        ...
        if (txt_jumlahBarang.getText().isEmpty()) return;
int jumlah = Integer.parseInt(txt_jumlahBarang.getText());
    ...
    private void txt_uangDiterimaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_uangDiterimaKeyReleased
        ...
        if (uangBayar >= totalPembayaran) {
           ...
           txt_uangKembalian.setText(String.valueOf(uangBayar - totalPembayaran));
    ...
    try {
        if (txt_totalPembayaran.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Total pembayaran masih kosong.");
            return;
        }
        if (txt_uangDiterima.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Uang diterima masih kosong.");
            return;
        }
        ...
        int idTransaksi = Integer.parseInt(txt_kodeTransaksi.getText().trim());
        int totalPembayaran = Integer.parseInt(txt_totalPembayaran.getText().trim());
        int uangDiterima = Integer.parseInt(txt_uangDiterima.getText().trim());
        int uangKembalian = txt_uangKembalian.getText().trim().isEmpty() ? 0 : Integer.parseInt(txt_uangKembalian.getText().trim());
        int idUser = Integer.parseInt(txt_idKasir.getText().trim());
        java.util.Date utilDate = new java.util.Date();
        java.sql.Date sqlDate = new java.sql.Date(utilDate.getTime());

        // ============================
        // TAMBAHAN: HITUNG KEUNTUNGAN
        // ============================
        int totalKeuntungan = 0;
        for (int i = 0; i < tbl_detailBarang.getRowCount(); i++) {
            int kodeProduk = Integer.parseInt(tbl_detailBarang.getValueAt(i, 0).toString());
            int jumlah = Integer.parseInt(tbl_detailBarang.getValueAt(i, 2).toString());
            int hargaJual = Integer.parseInt(tbl_detailBarang.getValueAt(i, 3).toString());

            // Ambil harga beli dari tabel produk
            String sqlHargaBeli = "SELECT harga_beli FROM produk WHERE kode_produk = ?";
            PreparedStatement psHargaBeli = conn.prepareStatement(sqlHargaBeli);
            psHargaBeli.setInt(1, kodeProduk);
            ResultSet rsHarga = psHargaBeli.executeQuery();
            int hargaBeli = 0;
            if (rsHarga.next()) {
                hargaBeli = rsHarga.getInt("harga_beli");
            }
            rsHarga.close();
            psHargaBeli.close();
        ...
        // Simpan ke tabel penjualan, tambahkan kolom keuntungan
        String sqlPenjualan = "INSERT INTO penjualan (penjualan_id, tanggal_penjualan, total_pembayaran, uang_diterima, uang_kembalian, user_id, keuntungan) "
                            + "VALUES (?,?,?,?,?,?,?)";
        PreparedStatement psPenjualan = conn.prepareStatement(sqlPenjualan);
        psPenjualan.setInt(1, idTransaksi);
        psPenjualan.setDate(2, sqlDate);
        psPenjualan.setInt(3, totalPembayaran);
        psPenjualan.setInt(4, uangDiterima);
        psPenjualan.setInt(5, uangKembalian);
        psPenjualan.setInt(6, idUser);
        psPenjualan.setInt(7, totalKeuntungan);
        psPenjualan.executeUpdate();
        psPenjualan.close();

        String detailSql = "INSERT INTO detail_penjualan (penjualan_id, kode_produk, jumlah, subtotal) VALUES (?,?,?,?)";
        PreparedStatement psDetail = conn.prepareStatement(detailSql);

        for (int i = 0; i < tbl_detailBarang.getRowCount(); i++) {
            int kodeProduk = Integer.parseInt(tbl_detailBarang.getValueAt(i, 0).toString());
            int jumlah = Integer.parseInt(tbl_detailBarang.getValueAt(i, 2).toString());
            int subtotal = Integer.parseInt(tbl_detailBarang.getValueAt(i, 4).toString());

            psDetail.setInt(1, idTransaksi);
            psDetail.setInt(2, kodeProduk);
            psDetail.setInt(3, jumlah);
            psDetail.setInt(4, subtotal);
            psDetail.addBatch();

            // update stok
            String updateStok = "UPDATE produk SET stok_produk = stok_produk - ? WHERE kode_produk = ?";
            PreparedStatement psUpdateStok = conn.prepareStatement(updateStok);
            psUpdateStok.setInt(1, jumlah);
            psUpdateStok.setInt(2, kodeProduk);
        ...
        conn.commit();
        JOptionPane.showMessageDialog(null, "Transaksi berhasil disimpan!\nKeuntungan transaksi ini: Rp " + totalKeuntungan);

        ((DefaultTableModel) tbl_detailBarang.getModel()).setRowCount(0);
        txt_totalPembayaran.setText("");
...
// --- akhir pengganti blok simpan detail ---
