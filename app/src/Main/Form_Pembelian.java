...
public class Form_Pembelian extends javax.swing.JPanel {
    ...
    + "JOIN merek ON produk.merek_Id = merek.merek_Id "
    + "WHERE produk.kode_produk = ? "
    + "OR produk.nama_produk LIKE ? "
    + "OR kategori.nama_kategori LIKE ? "
    + "OR merek.nama_merek LIKE ?";


ps = Koneksi.getConnection().prepareStatement(cari);

// cek apakah input user angka atau bukan
String keyword = txt_cariBarang.getText();
try {
    int kode = Integer.parseInt(keyword); // kalau bisa di-parse berarti angka
    ps.setInt(1, kode);
} catch (NumberFormatException e) {
    ps.setInt(1, -1); // kasih nilai yang pasti tidak ada
}
...
try {                                         
    if (tbl_detailBarang.getRowCount() == 0) {
        JOptionPane.showMessageDialog(null, "Detail barang masih kosong.");
        return;
    }
    ...
    String tglStr = txt_tanggalTransaksi.getText().trim();
    java.sql.Date sqlDate = java.sql.Date.valueOf(tglStr);

    // 1️⃣ Insert pembelian dulu (tanpa total)
    String sqlPembelian = "INSERT INTO pembelian (pembelian_id, tanggal_pembelian, user_id, total_pembelian) VALUES (?, ?, ?, 0)";
    PreparedStatement psPembelian = conn.prepareStatement(sqlPembelian);
    psPembelian.setInt(1, idTransaksi);
    psPembelian.setDate(2, sqlDate);
    psPembelian.setInt(3, idUser);
    psPembelian.executeUpdate();
    psPembelian.close();

    // 2️⃣ Insert detail pembelian
    String detailSql = "INSERT INTO detail_pembelian (pembelian_id, kode_produk, jumlah, subtotal) VALUES (?, ?, ?, ?)";
    PreparedStatement psDetail = conn.prepareStatement(detailSql);

    int totalPembelian = 0;

    for (int i = 0; i < tbl_detailBarang.getRowCount(); i++) {
        Object kodeObj = tbl_detailBarang.getValueAt(i, 0);
        Object jumlahObj = tbl_detailBarang.getValueAt(i, 2);

        if (kodeObj == null || jumlahObj == null) continue;

        String kodeStr = kodeObj.toString().trim();
        String jumlahStr = jumlahObj.toString().trim();
        if (kodeStr.isEmpty() || jumlahStr.isEmpty()) continue;

        int kodeProduk = Integer.parseInt(kodeStr);
        int jumlah = Integer.parseInt(jumlahStr);

        // Ambil harga beli dari produk
        String sqlHarga = "SELECT harga_beli FROM produk WHERE kode_produk = ?";
        PreparedStatement psHarga = conn.prepareStatement(sqlHarga);
        psHarga.setInt(1, kodeProduk);
    ...
    // 3️⃣ Update total pembelian setelah detail disimpan
    String updateTotal = "UPDATE pembelian SET total_pembelian = ? WHERE pembelian_id = ?";
    PreparedStatement psUpdate = conn.prepareStatement(updateTotal);
    psUpdate.setInt(1, totalPembelian);
    psUpdate.setInt(2, idTransaksi);
